...
# ╔═╡ 097967fc-aebe-4d92-832d-3ae5d92be89b
begin
	url = "https://archive.ics.uci.edu/ml/machine-learning-databases/mushroom/agaricus-lepiota.data"
	download(url, "agaricus-lepiota.data.csv")
	mushrooms_data = CSV.read("agaricus-lepiota.data.csv", DataFrame, header = header)
end
...
combine(groupby(mushrooms_data, "class"), :class => length => "# of mushroom")
...
# ╔═╡ fcdbdbe0-c6dd-42be-9873-df68a8b765fe
function unique_col(data::DataFrame)
	col_unique = Dict()
	# col_unique = zeros(size(data)[2])
	for (i, col) in enumerate(names(data))
		col_unique[col] = length(unique(data[:, col]))
	end
	return DataFrame(col_unique)
end

# ╔═╡ b2b7f399-645d-4d0b-ad54-6cde73a96cac
combine(mushrooms_data, unique_col)
...
# ╔═╡ b842bf1d-96f0-4930-adda-39ffbd7cd06f
transform!(mushrooms_data, :"class" => (x-> replace.(x, "e"=> "edible", "p"=>"poisonous")), renamecols=false)

# ╔═╡ 7d6741c5-c2ae-4514-b2b9-4afa24599637
transform!(mushrooms_data, :"odor" => (x-> replace.(x, "a" => "almond", "l" => "anise", "c" => "creosote", "y" => "fishy", "f" => "foul", "m" => "musty", "n" => "none", "p" => "pungent", "s" => "spicy")), renamecols=false)

# ╔═╡ 47bfa403-1f91-4859-b8de-bdc5ddc6e670
transform!(mushrooms_data, :"cap-shape" => (x-> replace.(x, "b" => "bell", "c" => "conical", "x" => "convex", "f" => "flat", "k" => "knobbed", "s" => "sunken")), renamecols=false)

# ╔═╡ b6de29fc-593b-40fa-be4c-362e41e27e86
transform!(mushrooms_data, :"cap-surface" => (x-> replace.(x, "f" => "fibrous", "g" => "grooves", "y" => "scaly", "s" => "smooth")), renamecols=false)

# ╔═╡ e89c306d-1516-45d5-87dc-2560b066f348
begin
	set_aog_theme!()

	axis = (width = 350, height = 350, ylabel="# of mushroom", xticklabelrotation=45)
	# xticks!(rotation=90)
	
	plt = data(mushrooms_data) * frequency() * mapping(:odor) * mapping(color = :class, dodge = :class)
	draw(plt; axis)
end
...
for col in names(mushrooms_data)
	if col!= "class"
		unique_features = unique(mushrooms_data[:, col])
		for feature in unique_features
			mushrooms_data[:, "$col $feature"] = Int64.(feature .== mushrooms_data[:, col])
		end
	end
end
...
X = Matrix(select(mushrooms_data, Not(header)))
...
y = Int64.("edible" .== mushrooms_data[:, "class"])
...
begin
	algo = RandomForestClassifier(oob_score=true)
	algo.fit(X, y)
end
...
depth = [algo.estimators_[i].get_depth() for i in 1:tree]

# ╔═╡ 8e3f2699-0e0b-41b5-a429-12256e91ba24
begin
	local fig = Figure()
	local ax = Axis(fig[1,1], xlabel = "Tree depth", ylabel = "# of occurence",
		   			xticks= unique(depth))
	ylims!(0, nothing)
	hist!(depth)
	fig
...
1. each tree is trained on a random subset of the original feature 
2. at each split within a tree, A fresh sample of
features is taken at each split
"
...
oob = algo.oob_decision_function_

# ╔═╡ 684e6ae0-415e-472f-b75f-0006a588db63
y_pred = Int64.(oob[:, 2] .> 0.5)

# ╔═╡ 58aec609-4381-4d4f-afd1-35b964c89f15
...
# ╔═╡ 27dbde9e-9b41-485c-98a3-187a325930b2
feature_name = names(select(mushrooms_data, Not(header)))

# ╔═╡ 6cd9000c-2c2a-4cef-b865-8186e0c9a441
begin
	order = DataFrame("feature" => feature_name, "Importance" => importance)
	order = sort(order, :Importance, rev=true)[1:10, :]
end

# ╔═╡ 582ac7bc-e79c-4c93-b948-7c19a4ba4924
begin
	data_size = size(order)[1]
	local fig = Figure()
	local ax = Axis(fig[1, 1], ylabel = "Feature", xlabel ="Importance", 
				    yticks = (1:data_size, order[:, "feature"]))
	barplot!(1:data_size, order[:, "Importance"], direction=:x)
	fig
end
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "21c34b1eaa9d87795396ac874376c5d54b05372b"
